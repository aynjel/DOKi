...
export class TabAppointmentsPage {
    ...
    const options1: GestureConfig = {
      ...
      onMove: (ev) => {
        ...
        if (ev.deltaX > 120) {
          ...
          this.yAxisArray[1].nativeElement.style.transform = `translateX(${
            ev.deltaX
          }px) rotate(${ev.deltaX / 90}deg)`;
        }
      },
      onEnd: (ev) => {
        ...
        if (ev.deltaX > 120) {
          this.zone.run(() => {
            this.headerData = this.selectedDate =
              this.functionsService.incrementDate(this.selectedDate, -1);
  ...
  async presentActionSheet(data1: any, data2: any, data3: any) {
    if (
      data2 == this.constants.APPOINTMENT_STATUS__VALUE__RESERVED /*"Reserved"*/
    ) {
      const actionSheet = await this.actionSheetController.create({
        ...
        buttons: [
          ...
          {
            text: this.constants.UI_COMPONENT_TEXT__VALUE__CANCEL,
            icon: 'close',
            role: 'cancel',
          },
        ],
  ...
  async presentAlertConfirm(data1: any, data2: any) {
    const alert = await this.alertController.create({
      cssClass: 'my-custom-class',
      message:
        this.messages.CONFIRMATION_DELETE_RECORD +
        '<strong>' +
        data1 +
  ...
  adjustDate(data1: any) {
    this.selectedDate = this.functionsService.incrementDate(
      this.selectedDate,
      data1
    );
    this.getDate(this.selectedDate, this.selectedLocation);
